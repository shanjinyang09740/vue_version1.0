<template>
	<div class="fillcontain">
		<div class="table_container">
			<el-table :data="tabDataArr" @expand="expand" :row-key="row => row.index" :expand-row-keys="expendRow" style="width: 100%">
			...
			<div class="Pagination">
				<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage"
				 :page-size="pageSize" layout="total, prev, pager, next" :total="count">
				</el-pagination>
			</div>
	...
	export default {
		...
		methods: {
			...
			async getMenu() {
				this.menuOptions = [];
				try {
					const menu = await getCategory(this.selectTable.restaurant_id);
					menu.data.category_list.forEach((item, index) => {
			...
			async getFoods() {
				...
				this.dataList.forEach((item, index) => {
					...
					tableData.category_name = "特卖商品";
					const resturantdetail = Resturant.data.filter(
						(i, index1) => i.restaurant_id == item.restaurant_id
					);
					if (resturantdetail.length !== 0) {
						const restaurant = resturantdetail[0];
						tableData.restaurant_name = restaurant.name;
						tableData.restaurant_address = restaurant.address;
						tableData.restaurant_address = restaurant.address;
					}
			...
			async getSelectItemData(row, type) {
				const result = await getResturantDetail(row.restaurant_id);
			...
			async handleDelete(index, row) {
				...
				try {
					...
					if (res.code == 0) {
						...
						this.tabDataArr.splice(index, 1);
				...
				try {
					...
					if (res.code == 0) {
						...
						const index = this.tableData.findIndex(
							(item, index) => item.item_id === postData.item_id
						);
						this.tableData.splice(index, 1, postData);
					} else {
						this.$message({
							type: "error",
							message: res.message
