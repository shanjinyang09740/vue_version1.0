      ...
      this.$refs[formName].validate(async valid => {
        if (valid) {
          ...
          if (userArr.indexOf(this.loginForm.username) > -1) {
            ...
            if (this.adminInfo[curIndex]["username"] === "admin") {
              const auth = JSON.stringify("edit");
              sessionStorage.setItem("Auth_key", auth); //存入一个值key:value
            }else{
              const auth = JSON.stringify("no");
              sessionStorage.setItem("Auth_key", auth); //存入一个值key:value
            }
           
            if (
              this.loginForm.password === this.adminInfo[curIndex]["password"]
            ) {
              sessionStorage.setItem("Check_login", "yes");
              this.$message({
                type: "success",
                message: "登录成功"
              });
